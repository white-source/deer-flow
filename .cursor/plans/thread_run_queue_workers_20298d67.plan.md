...
**Notes:** [`docs/notes/task驱动/多轮对话/thread_run_queue_workers_20298d67.md`](docs/notes/task驱动/多轮对话/thread_run_queue_workers_20298d67.md)
...
**已实现行为：** 见 notes 文档 Task 6 表格（`submitEnqueuedRun` → `pendingJoinRunsRef` → `drainPendingJoinRuns` → `joinStream`）。

**已知限制：** Stop 仅作用于当前流；queued 单独 cancel 的 UI 未做；E2E 连发两条完整流式待补。

- [x] **Step 1: Unit test** — [`run-queue.test.ts`](frontend/tests/unit/core/threads/run-queue.test.ts)

- [ ] **Step 2: E2E** — [`chat.spec.ts`](frontend/tests/e2e/chat.spec.ts) 第二条完整 SSE

- [x] **Step 3-5: Implement** — 含 202 后 `joinStream` 修复
...
| 手动回归 | 同会话 Run1 流式中发 Run2 → Run2 应完整流式 |
...
| 用户持续发消息 | Task 6 | ✅ |
| 消息进入任务队列 | Task 2-4 | ✅ |
| 后台多个 Worker | Task 4/7 | ✅ |
| 不再锁死会话 | Task 5 + 6 | ✅ |
| 排队 Run 前端 SSE | Task 6 join 修复 | ✅ |
| 同线程安全 | MVP 串行 | ✅ |
...
| 日期 | 变更 |
|------|------|
| Phase 1–2 | 后端 queue + dispatcher + 202 |
| Phase 1 Task 6 | 前端非阻塞 + badge |
| 2026-06-04 | `waitUntilRunJoinable` + `thread.joinStream` 修复第二条消息流式 |
